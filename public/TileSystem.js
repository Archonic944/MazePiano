...
class TileSystem {
  constructor(state = null) {
    console.log("Filling revealedTiles to false");
    // Base fields
    this.tileSize = 32; // Same as player width/height
    this.sprites = {};
    this.tiles = {}; // Will store tiles as "x,y" -> tileType
    this.revealedTiles = Array.from({ length: CHAMBER_WIDTH }, () =>
      Array(CHAMBER_HEIGHT).fill(false)
    );
    this.loadSprites();
    // Hydrate from provided state (from network) or generate a new maze locally
    if (state) {
  ...
  // Apply a plain-object state (e.g., received over WebSocket) to this instance
  applyState(state) {
    if (!state || typeof state !== "object") return;
    if (typeof state.tileSize === "number") this.tileSize = state.tileSize;
    if (state.tiles && typeof state.tiles === "object")
      this.tiles = { ...state.tiles };
    if (Array.isArray(state.revealedTiles)) {
      // Deep copy to avoid sharing references across clients
      console.log("Hydrating revealedTiles");
      this.revealedTiles = state.revealedTiles.map((col) =>
        Array.isArray(col) ? col.slice() : col
      );
    }
  }

  // Produce a minimal, serializable snapshot of this tile system (no Image objects)
  getState() {
    return {
  ...
  generateMaze() {
    ...
    let getAdjacent = function (tile) {
      // Times two because air tiles are at odd coordinates
      const [x, y] = tile;
      return [
        [x + 2, y],
        [x, y + 2],
    ...
    let filterTile = function (tile) {
      // Filter by in bounds and not visited
      const [x, y] = tile;
      return (
        x >= 0 &&
        x < CHAMBER_WIDTH &&
    ...
    let visited = Array.from({ length: CHAMBER_WIDTH }, () =>
      Array(CHAMBER_HEIGHT).fill(false)
    ); // this is AI i have no idea why it works
    let initialCell = [1, 1];
    ...
    for (let tileX = startTileX; tileX <= endTileX; tileX++) {
      for (let tileY = startTileY; tileY <= endTileY; tileY++) {
        ...
        if (tileType === TILE_TYPES.COIN) {
          ...
          tileX >= 0 &&
          tileX < this.revealedTiles.length &&
          tileY >= 0 &&
          tileY < this.revealedTiles[0].length &&
